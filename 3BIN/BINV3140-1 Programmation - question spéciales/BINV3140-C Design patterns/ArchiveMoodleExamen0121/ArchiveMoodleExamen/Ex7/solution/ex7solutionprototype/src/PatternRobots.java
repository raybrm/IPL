public class PatternRobots {
	...
	//	Robot robot2=new DegatDivisePar2(new BouclierMultipliePar2(new RobotImpl.RobotBuilder("pol").canon(9).shield(3).freq(90).build()));
		...
		Robot r=new RobotImpl.RobotBuilder("grosse Berta").canon(100).build();
		Robot r2=new RobotImpl.RobotBuilder("pic vert").freq(100).canon(2).build();
		ff.putFlyweight("grosse Berta", new BouclierMultipliePar2(r));
		ff.putFlyweight("picvert", new CanonMultipliePar2(r2));
		Robot robot1=ff.createRobot("picvert");
		Robot robot2=ff.createRobot("picvert");
		fight(robot1, robot2);
		 robot1=ff.createRobot("picvert");
		robot2=ff.createRobot("picvert");
		fight(robot1, robot2);
