...
// Abstract Factory pour le 2ème pattern
		...
		try {
			...
			while (fichier.hasNextLine()) {
				try {
					...
					CarteDeCredit carteDeCredit = new AmericanExpressHandler(new DinersClubHandler(new DiscoverHandler(new MasterCardHandler(new VisaHandler(null))))).creerCarte(parts[0], dateExpiration, parts[2]); //???????(parts[0], dateExpiration, parts[2]); // creation chaine de responsabilitÃ©
